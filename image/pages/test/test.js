const { questionList } = require("../../data/json");
...
Page({
  ...
  */
  chooseError: function() {
    let chooseVal = this.data.chooseValue[this.data.index];
    let tmp = this.data.chooseValue[this.data.index] + "_ANS"
    let trueValue = this.data.questionList[this.data.shuffleIndex[this.data.index]][tmp];
    console.log('选择了' + chooseVal + '答案是' + trueValue);
    this.setData({
      totalScore: this.data.totalScore + trueValue  // 加分操作
    })
  },
  /**
